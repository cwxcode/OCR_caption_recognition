...
test_dataset = dataset.lmdbDataset(
    root=opt.valroot, transform=dataset.resizeNormalize((200, 64)))
...
if opt.crnn != '':
    ...
    load_pretrained_model(crnn, pre_trainmodel)
...
#        print (epoch+1,i)
        ...
        if i % opt.displayInterval == 0:
            print('[%d/%d][%d/%d] Loss: %f' %
                  (epoch+1, opt.niter, i, len(train_loader), loss_avg.val()))
            loss_avg.reset()
        ...
        if i % opt.saveInterval == 0:
            torch.save(
                crnn.state_dict(), '{0}/netCRNN_{1}_{2}.pth'.format(opt.experiment, epoch+1, i))
